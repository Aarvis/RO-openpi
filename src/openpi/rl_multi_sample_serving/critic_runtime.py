            ...
            with torch.inference_mode():
                with _make_autocast_context(self._device, self._amp_dtype):
                    prediction = self._model(
                        torch.as_tensor(np.asarray(policy_latent), dtype=torch.float32, device=self._device),
                        torch.as_tensor(np.asarray(state), dtype=torch.float32, device=self._device),
                        torch.as_tensor(np.asarray(action_chunk), dtype=torch.float32, device=self._device),
                    ).squeeze(-1)
                scores = prediction.detach().cpu().numpy().astype(np.float32, copy=False)
        return {
            "scores": scores,
            "critic_timing": {
