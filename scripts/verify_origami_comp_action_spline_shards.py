...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify Origami comp action-spline shards against source videos, arrays, spline targets, and planner exports."
    )
...
def _read_live_sample(
    ...
    *,
    ...
    with np.load(arrays_root / settings.local_target_npz_name, allow_pickle=False) as target_archive:
        chunk, action_mask = _shards.pack_spline_target(
            target_archive,
    ...
    if settings.load_tactile_images:
        ...
        output["tactile_deform_available"] = np.asarray(tactile_available, dtype=bool)
...
def main() -> int:
    ...
    shard_dataset = _shards.OrigamiCompActionSplineShardDataset(settings, split=args.split)
    ...
    print("Origami comp action-spline shard verification")
    ...
    if failures:
        ...
        for failure in failures[: args.max_failures]:
            print(f"- {failure}")
        return 1
    print("\nOK: sampled action-spline shard rows match source videos, spline targets, planner features, masks, and weights.")
    return 0
