...
import openpi.training.origami_comp_action_spline_shards as _shards
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build uncompressed, memory-mappable Origami pi0.5 comp action-spline shards. "
...
def _write_plan_files(
    *,
    ...
    manifest = {
        "format": "origami_comp_action_spline_uncompressed_shards_v1",
        ...
        "settings": {
            ...
            "action_source": str(settings.action_source),
            "local_target_npz_name": str(settings.local_target_npz_name),
            "local_target_index_name": str(settings.local_target_index_name),
            ...
            "tactile_raw_input_dropout_prob": float(settings.tactile_raw_input_dropout_prob),
            "tactile_raw_dropout_seed": int(settings.tactile_raw_dropout_seed),
            "tactile_image_input_dropout_prob": float(settings.tactile_image_input_dropout_prob),
            "tactile_image_dropout_seed": int(settings.tactile_image_dropout_seed),
...
def _array_specs(
    ...
    *,
    ...
    if settings.load_tactile_images:
        ...
        specs["tactile_deform_available"] = (np.bool_, (num_rows,))
...
def _build_one_shard(task: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for episode_uid, episode_rows in rows.groupby("episode_uid", sort=False):
            ...
            try:
                ...
                for row in episode_rows.to_dict(orient="records"):
                    ...
                    chunk, mask = _shards.pack_spline_target(
                        target_archive,
                        int(row["local_target_npz_sample_index"]),
                        settings,
                    )
                    ...
                    if settings.load_tactile_images:
                        ...
                        tactile_available = not _shards._drop_tactile_image_input(settings, episode_uid, frame_position)
                        ...
                        arrays["tactile_deform_available"][local_row] = np.asarray(tactile_available, dtype=bool)
                ...
                close_target_archive = getattr(target_archive, "close", None)
                if close_target_archive is not None:
                    close_target_archive()
            if pending_progress:
                _progress_put(
                    progress_queue,
                    "frames",
        ...
        metadata = {
            "format": "origami_comp_action_spline_uncompressed_shard_v1",
            "split": split,
            "shard_id": shard_id,
            "shard_name": shard_name,
            "num_rows": num_rows,
...
def main() -> int:
    ...
    print("Origami comp action-spline shard build")
    ...
    for split in split_names:
        ...
        print(
            f"{split} target layout : actions=({settings.action_horizon}, {settings.action_dim}) "
            f"control_points={settings.max_control_points} spans={settings.max_span_count}"
        )
        if args.plan_only:
            continue
